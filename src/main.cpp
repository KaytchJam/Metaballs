...
// template <int v_size>
// void print_vec(const glm::vec<v_size, float, glm::packed_highp>& v) {
//     std::cout << "[";
...
void mouse_callback(GLFWwindow* window, double xpos_d, double ypos_d) {
    if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) && glfwGetWindowAttrib(window, GLFW_HOVERED) == GLFW_FALSE)
        return;
